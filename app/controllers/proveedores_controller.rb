class ProveedoresController < ApplicationController
  before_action :set_proveedor, only: [:mostrar, :editar, :update, :eliminar]

  def index
    @proveedores = Provedor.paginate(:page => params[:page], :per_page => 10)
  end

  def crear
    @proveedor = Provedor.new(proveedor_params)
    respond_to do |format|
      if @proveedor.save
        format.html{redirect_to @proveedor, notice:  'Proveedor Guardado Con Exito'}
      else
        format.html{render :new}
      end
    end
  end

  def editar
  end

  def mostrar
  end

  def nuevo
    @proveedor = Provedor.new
  end


  def update
    respond_to do |format|
      if @proveedor.update(proveedor_params)
        format.html{redirect_to proveedor_url(@proveedor) , notice: 'Proveedor Editado con Exito'}
      else
        format.html{render :edit}
      end
    end
  end

  def eliminar
    @proveedor.destroy
    respond_to do |format|
      format.html { redirect_to proveedor_url, notice: 'Proveedor eliminado con Exito.' }
    end
  end



  private
  # Use callbacks to share common setup or constraints between actions.
  def set_proveedor
    @proveedor = Provedor.find(params[:id])
  end

  # Never trust parameters from the scary internet, only allow the white list through.
  def proveedor_params
    params.require(:provedor).permit(:nombre)
  end

end
